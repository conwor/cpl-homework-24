# Домашнее задание №24 - Реализация динамического массива макросами (4 балла)

Реализуйте с помощью function-like макросов, параметризованных треми параметрами `T`, `NAME` и
`DEFAULT_CAP`, шаблон структуры динамического массива произвольного типа. Параметр `T` определяет
тип элемента, `NAME` - имя структуры, `DEFAULT_CAP` - начальную ёмкость по умолчанию.
...
* `void NAME##_init(NAME* arr)` - инициализация массива, созданного в произвольной памяти, с
  начальной ёмкостью `DEFAULT_CAP`
...
* `T NAME##_get_or(NAME* arr, size_t index, T _default)` - возврат элемент по индексу, либо
  `_default`, если индекс не входит в пределы массива
